...
#
# Company ---> Rompetrol
# Link ------> https://careers.rompetrol.com/search/?createNewAlert=false&q=&optionsFacetsDD_country=RO&optionsFacetsDD_dept=&locationsearch=
#
#
from __utils import (
    GetStaticSoup,
...
def scraper():
    '''
    ... scrape data from Rompetrol scraper.
    '''
    soup = GetStaticSoup("https://careers.rompetrol.com/search/?createNewAlert=false&q=&optionsFacetsDD_country=RO&optionsFacetsDD_dept=&locationsearch=")

    job_list = []
    html_data = soup.find_all('tr', class_='data-row')
    for job in html_data:
        link = 'https://careers.rompetrol.com/' + job.find('a', class_='jobTitle-link').get('href')
